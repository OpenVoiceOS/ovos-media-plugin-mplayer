...
class BaseMplayerCtrlException(Exception):
    ...
    def __str__(self):
        ret = ', '.join(self.args)
        LOG.error('Exception: <{}>: {}'.format(self.__class__.__name__, ret))
        return ret
...
class MplayerStderrEvents(threading.Thread):
    ...
    * EVT_STDERR'''
    ...
    def run(self):
        while self._stdout_event_thread.is_alive():
...
class MplayerCtrl:
    ...
    def _run_cmd(self, cmd, *args):
        if not self.process_alive:
            raise NoMplayerRunning('You have first to start the mplayer,'
                                   'use Start()')
        args = ' '.join(str(i).replace(' ', '\\ ')
                        for i in args if not i is None)
        args.replace('\\', '\\\\')  # escape escaped backslashes :D
        if self.debug:
            LOG.debug('{!r}, {!r}'.format(cmd, args), 'running command')

        if self.keep_pause:
            stostdin = 'pausing_keep {} {}\n\n'.format(cmd, args)
        else:
            stostdin = '{} {}\n\n'.format(cmd, args)
    ...
    def get_property(self, property):
        ...
        self._run_cmd('get_property', property)
    ...
    def loadfile(self, file_url, append=None):
        ...
        self._run_cmd('loadfile', http_ffmpeg_file, append)
            ...
            if prop['set']:
                ...
                if not min <= value <= max:
                    raise AnsError('value must be between %.2f and %.2f' %
                                   (min, max))
                self._run_cmd('set_property', property, value)
            else:
